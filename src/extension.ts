...
export function activate(context: vscode.ExtensionContext) {
    ...
    if (ConfigManager.isDebugMode()) {
        console.log(
            `[启动延迟] 将在 ${ConfigManager.getStartupDelay()} 秒后启动文件装饰提供者`,
        );
    }
    ...
    const startupTimer = setTimeout(() => {
        ...
        if (ConfigManager.isDebugMode()) {
            // 输出成功注册的调试信息
            console.log(
                `[延迟启动完成] 文件装饰提供者已注册，总订阅数: ${context.subscriptions.length}`,
            );
        }
    }, startupDelay);
