...
export class Formatters { // 格式化工具类，封装各种数据格式化逻辑，支持国际化和模板替换
    private static readonly SIZE_UNITS: FileSizeUnits = { // 文件大小单位定义
    ...
    public static renderTemplate(template: string, variables: TemplateVariables): string { // 渲染模板字符串，将模板中的占位符替换为实际值
        let result = template;
        result = result.replace(/{name}/g, variables.name || ''); // 替换基本变量
        result = result.replace(/{modifiedTime}/g, variables.modifiedTime || '');

        if (variables.size !== undefined) { // 替换可选变量
        ...
        if (variables.resolution !== undefined) { // 替换图片分辨率相关变量，如果变量未定义则移除整个占位符
    ...
    public static formatImageResolution(dimensions: ImageDimensions, template?: string): string { // 格式化图片分辨率信息
    ...
    public static createFileVariables( // 创建文件的模板变量对象
        fileName: string,
        fileSize: number,
        modifiedTime: Date,
        ...
        if (imageDimensions) { // 如果是图片文件且有尺寸信息，添加分辨率相关变量
    ...
    public static createFolderVariables( // 创建文件夹的模板变量对象
        folderName: string,
        size: number,
        fileCount: number,
        folderCount: number,
    ...
    public static createCalculatingVariables( // 创建文件夹计算中状态的模板变量对象
        folderName: string,
        modifiedTime: Date
    ): TemplateVariables {
        return {
    ...
    public static createTimeoutVariables( // 创建文件夹超时状态的模板变量对象
        folderName: string,
        modifiedTime: Date,
